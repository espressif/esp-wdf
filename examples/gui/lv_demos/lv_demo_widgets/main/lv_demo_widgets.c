...
({                                                                               \
    int _ret = lv_draw_rect_dsc_get_data(o, t, &p, sizeof(p));              \
    if (_ret) {                                                                 \
        LV_WRAPPER_DEBUG();                                                                \
    }                                                                           \
    _ret; \
...
({                                                                               \
    int _ret = lv_draw_rect_dsc_set_data(o, t, &p, sizeof(p));              \
    if (_ret) {                                                                 \
        LV_WRAPPER_DEBUG();    \
    }                                                                           \
    _ret; \
...
#define LV_DRAW_DSC_BASE_GET_DATA(o, t, p)                                  \
({                                                                               \
    int _ret = lv_draw_dsc_base_get_data(o, t, &p, sizeof(p));              \
    if (_ret) {                                                                 \
        LV_WRAPPER_DEBUG();                                                                \
    }                                                                           \
    _ret; \
...
#define LV_DRAW_DSC_BASE_SET_DATA(o, t, p)                                  \
({                                                                               \
    int _ret = lv_draw_dsc_base_set_data(o, t, &p, sizeof(p));              \
    if (_ret) {                                                                 \
        LV_WRAPPER_DEBUG();    \
    }                                                                           \
    _ret; \
...
#define LV_DRAW_LINE_DSC_GET_DATA(o, t, p)                                  \
({                                                                               \
    int _ret = lv_draw_line_dsc_get_data(o, t, &p, sizeof(p));              \
    if (_ret) {                                                                 \
        LV_WRAPPER_DEBUG();                                                                \
    }                                                                           \
    _ret; \
...
#define LV_DRAW_LINE_DSC_SET_DATA(o, t, p)                                  \
({                                                                               \
    int _ret = lv_draw_line_dsc_set_data(o, t, &p, sizeof(p));              \
    if (_ret) {                                                                 \
        LV_WRAPPER_DEBUG();    \
    }                                                                           \
    _ret; \
...
#define LV_DRAW_FILL_DSC_GET_DATA(o, t, p)                                  \
({                                                                               \
    int _ret = lv_draw_fill_dsc_get_data(o, t, &p, sizeof(p));              \
    if (_ret) {                                                                 \
        LV_WRAPPER_DEBUG();    \
    }                                                                           \
    _ret; \
...
void lv_demo_widgets(void)
{
    ...
    if(disp_size == DISP_LARGE) {
        ...
        font_large     = lv_font_get_font(LV_FONT_BENCHMARK_MONTSERRAT_24_COMPR_AZ_FONT);
...
#endif
#if LV_DEMO_BENCHMARK_ALIGNED_FONTS
        font_normal    = lv_font_get_font(LV_FONT_BENCHMARK_MONTSERRAT_16_COMPR_AZ_FONT);
        ...
        tab_h = 45;
#if LV_DEMO_BENCHMARK_ALIGNED_FONTS
        font_large     = lv_font_get_font(LV_FONT_BENCHMARK_MONTSERRAT_20_COMPR_AZ_FONT);
...
#endif
#if LV_DEMO_BENCHMARK_ALIGNED_FONTS
        font_normal    = lv_font_get_font(LV_FONT_BENCHMARK_MONTSERRAT_14_COMPR_AZ_FONT);
        ...
        tab_h = 45;
#if LV_DEMO_BENCHMARK_ALIGNED_FONTS
        font_large     = lv_font_get_font(LV_FONT_BENCHMARK_MONTSERRAT_18_COMPR_AZ_FONT);
...
#endif
#if LV_DEMO_BENCHMARK_ALIGNED_FONTS
        font_normal    = lv_font_get_font(LV_FONT_BENCHMARK_MONTSERRAT_12_COMPR_AZ_FONT);
...
static void analytics_create(lv_obj_t * parent)
{
    ...
    meter2_timer = lv_timer_create(scale2_timer_cb, 100, scale2);
    lv_obj_add_event_cb(scale2, delete_timer_event_cb, LV_EVENT_DELETE, meter2_timer);
...
static void slider_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
        ...
        LV_OBJ_DRAW_PART_DSC_GET_DATA(draw_rect_dsc, LV_DRAW_RECT_DSC_BASE, base);

        if(base.part == LV_PART_KNOB && lv_obj_has_state(obj, LV_STATE_PRESSED)) {
            char buf[8];
            ...
            lv_draw_rect(base.layer, &rect_dsc, &bg_area);
            ...
            lv_draw_label(base.layer, &label_dsc, &txt_area);
        }
    }
}
...
static void chart_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_PRESSED || code == LV_EVENT_RELEASED) {
        ...
        uint32_t id2;
        lv_layer_t layer;
        LV_DRAW_DSC_BASE_GET_DATA(base_dsc, LV_DRAW_DSC_BASE_PART, part);
        LV_DRAW_DSC_BASE_GET_DATA(base_dsc, LV_DRAW_DSC_BASE_ID1, id1);
        LV_DRAW_DSC_BASE_GET_DATA(base_dsc, LV_DRAW_DSC_BASE_ID2, id2);
        LV_DRAW_DSC_BASE_GET_DATA(base_dsc, LV_DRAW_DSC_BASE_LAYER, layer);

        if(part == LV_PART_ITEMS && draw_line_dsc) {
            lv_area_t obj_coords;
            lv_obj_get_coords(obj, &obj_coords);
            const lv_chart_series_t * ser = lv_chart_get_series_next(obj, NULL);
            if(id1 == 1) ser = lv_chart_get_series_next(obj, ser);

            lv_draw_triangle_dsc_t tri_dsc;
            lv_draw_triangle_dsc_init(&tri_dsc);

            lv_point_precise_t p1, p2;
            LV_DRAW_LINE_DSC_GET_DATA(draw_line_dsc, LV_DRAW_LINE_DSC_P1, p1);
            LV_DRAW_LINE_DSC_GET_DATA(draw_line_dsc, LV_DRAW_LINE_DSC_P2, p2);
        ...
        if(part == LV_PART_INDICATOR && lv_chart_get_pressed_point(obj) == id2) {
            if(lv_chart_get_type(obj) == LV_CHART_TYPE_LINE) {
                ...
                lv_area_t draw_task_area;
                lv_draw_task_get_area(draw_task, &draw_task_area);
                lv_draw_rect(&layer, &outline_dsc, &draw_task_area);
                add_value = true;
            }
        }
        if(part == LV_PART_ITEMS && lv_chart_get_pressed_point(obj) == id2) {
            const lv_chart_series_t * ser = lv_chart_get_series_next(obj, NULL);
            if(id1 == 1) ser = lv_chart_get_series_next(obj, ser);
            ...
            if(lv_chart_get_type(obj) == LV_CHART_TYPE_BAR) {
                ...
                int32_t radius;
                LV_DRAW_FILL_DSC_GET_DATA(fill_dsc, LV_DRAW_FILL_DSC_RADIUS, radius);
                ...
                lv_area_t draw_task_area;
                lv_draw_task_get_area(draw_task, &draw_task_area);
                lv_draw_rect(&layer, &shadow_dsc, &draw_task_area);
                add_value = true;
            }
        }
        ...
        if(add_value) {
            ...
            char buf[8];
            lv_snprintf(buf, sizeof(buf), "%"LV_PRId32, lv_chart_get_series_y_array(obj, (lv_chart_series_t *)ser)[id2]);
