...
static void slider_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
        ...
        if(part == LV_PART_KNOB && lv_obj_has_state(obj, LV_STATE_PRESSED)) {
            ...
            LV_OBJ_DRAW_PART_DSC_GET_DATA(dsc, LV_OBJ_DRAW_PART_DSC_DRAW_CTX, draw_ctx);

            lv_draw_rect((lv_draw_ctx_t *)&draw_ctx, &rect_dsc, &bg_area);

            lv_draw_label_dsc_t label_dsc;
            lv_draw_label_dsc_init(&label_dsc);
            label_dsc.color = lv_color_white();
            label_dsc.font = font_normal;
            lv_draw_label((lv_draw_ctx_t *)&draw_ctx, &label_dsc, &txt_area, buf, NULL);
        }
    }
}
    ...
    if(code == LV_EVENT_PRESSED || code == LV_EVENT_RELEASED) {
        ...
        /*Add the faded area before the lines are drawn */
            ...
            if(!ret) {
                ...
                const lv_area_t *clip_area_ori = &clip_area;
                LV_OBJ_DRAW_PART_DSC_SET_DATA(dsc, LV_OBJ_DRAW_PART_DSC_CLIP_AREA, obj_clip_area);
                ...
                lv_draw_rect((lv_draw_ctx_t *)&draw_ctx, &draw_rect_dsc, &a);
                LV_OBJ_DRAW_PART_DSC_GET_DATA(dsc, LV_OBJ_DRAW_PART_DSC_CLIP_AREA, clip_area_ori);
                /*Remove the masks*/
