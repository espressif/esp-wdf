...
static void auto_step_cb(lv_timer_t * t)
{
    ...
    const lv_font_t * font_small = lv_font_get_font(LV_FONT_MONTSERRAT_22_FONT);
    const lv_font_t * font_large = lv_font_get_font(LV_FONT_MONTSERRAT_32_FONT);
#else
    const lv_font_t * font_small = lv_font_get_font(LV_FONT_MONTSERRAT_12_FONT);
    const lv_font_t * font_large = lv_font_get_font(LV_FONT_MONTSERRAT_16_FONT);
#endif
    ...
    switch(state) {
        ...
        case 40: {
                ...
                const lv_sysmon_perf_info_t * info = lv_subject_get_pointer(&sysmon_backend.subject);
                calculated_t calc; 
                lv_get_sys_perf_data(info, LV_SYS_PERF_INFO_CALC, &calc, sizeof(calculated_t))
                lv_label_set_text_fmt(num, "%" LV_PRIu32, calc.fps_avg_total);
#endif
