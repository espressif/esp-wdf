...
static void benchmark_init(void)
{
    ...
    if(run_max_speed) {
        ...
        int ret = lv_disp_get_data(disp, &refr_timer, sizeof(lv_timer_t));
        if(!ret) {
            disp_ori_timer_period = refr_timer.period;
            lv_timer_set_period(&refr_timer, 1);
        }

        lv_timer_t * anim_timer = lv_anim_get_timer();
        uint32_t period;
        ret = lv_anim_timer_get_data(anim_timer, &period, sizeof(uint32_t));
        if(!ret) {
            anim_ori_timer_period = period;
