...
[ -z ${BIN_PATH} ] && die "BIN_PATH is not set"
[ -d ${LOG_PATH} ] || mkdir -p ${LOG_PATH}
[ -d ${BIN_PATH} ] || mkdir -p ${BIN_PATH}

mkdir -p ${BIN_PATH}/wasm
...
EXAMPLE_PATHS=$( find ${IDF_PATH}/examples/ -type f -name CMakeLists.txt |grep -v "/common/" |grep -v "/components/" | grep -v "/common_components/" | grep -v "/main/" | grep -v "/build_system/cmake/" | grep -v "/mb_example_common/" | sort )
if [ $# -eq 0 ]
then
    START_NUM=0
    END_NUM=999
...
build_example () {
    ...
    if [[ -f "example_builds/${EXAMPLE_BUILD_DIR}/build/ci_build_success" ]]; then
        ...
        if [[ "${EXAMPLE_NAME}" == "switch" ]]; then
            cp -r "${EXAMPLE_DIR}/../common/"* "example_builds/common"
        fi
